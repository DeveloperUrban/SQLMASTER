...
---AHORA VEREMOS LA FUNCION DE MIN Y MAX

---LA FUNCION MIN: La función MIN sirve para obtener el valor más pequeño para una columna determinada.
---Ejemplos:
---Se pide determinar la paciendo con la minima edad
select * from tbl_Paciente


select nombre MIN(DATEDIFF(YEAR,fNacimiento,GETDATE())) as Edad from tbl_Paciente GROUP BY nombre

CREATE PROCEDURE MODTISTOCK AS

SELECT AL.DESALMACEN, AL.CODALMACEN,L.DESLINEA,SL.DESSUBLINEA,AT.CODARTI,A.DESARTI, AT.STKACTU AS CANTIDAD,COST.COSTOBAS,Pres.despres,MAX(AT.ULTSAL),(AT.STKACTU*COST.COSTOBAS) as CostoTotal
FROM ARTIDAT AT
INNER JOIN ARTICULO A ON AT.CODARTI=A.CODARTI
INNER JOIN LINEA L ON A.CODLINEA=L.CODLINEA
INNER JOIN SUBLINEA SL ON A.CODSUBLINEA=SL.CODSUBLINEA
INNER JOIN ALMACEN AL ON AT.CODALMACEN=AL.CODALMACEN
LEFT JOIN (select * from [REPORT].dbo.COST_ART where CONVERT(date,FECDOCUM,103) = CONVERT(date,GETDATE(),103)) COST ON AT.CODARTI=COST.CDARTICULO
LEFT JOIN (SELECT CODARTI,despres,EQUIVALENCIA FROM ARTIPRESENT AP INNER JOIN PRESENTACION P ON AP.CODPRES=P.codpres WHERE EQUIVALENCIA=1) Pres ON pres.CODARTI=A.CODARTI
WHERE AT.CODALMACEN=001  AND AT.ULTSAL BETWEEN '01/01/2019' AND GETDATE()  
GROUP BY  AL.DESALMACEN, AL.CODALMACEN,L.DESLINEA,SL.DESSUBLINEA,AT.CODARTI,A.DESARTI,AT.STKACTU,COST.COSTOBAS,Pres.despres
